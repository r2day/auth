...
func (m *Model) GetList(ctx context.Context, merchantID string, accountID string, urlParams *rest.UrlParams) ([]*Model, int64, error) {
	...
	// 以商户id为基本命名空间
	// 并且只能看到小于等于自己的级别的数据
	filters := bson.D{{Key: "meta.merchant_id", Value: merchantID}, {"meta.access_level",
		bson.D{{"$lte", m.Meta.AccessLevel}}}}
	// 添加更多过滤器
	// 根据用户规则进行筛选
	...
	if urlParams.HasFilter {
		...
		// 添加分类过滤
		filterByCategory := bson.E{Key: "category", Value: urlParams.FilterCommon.CategoryID}
		filters = append(filters, filterByCategory)
	}
